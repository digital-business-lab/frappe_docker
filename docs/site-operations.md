...
```sh
docker-compose exec backend bench new-site --project-name <project-name> --no-mariadb-socket --mariadb-root-password <db-password> --admin-password <admin-password> --install-app erpnext --set-default <site-name>
```
